...
public:
    ...
    static constexpr u64 ExtractBits(u64 word, size_t page_start, size_t page_end) {
        ...
        if (page_start >= number_bits) [[unlikely]] return 0;
        if (page_end >= number_bits) [[unlikely]] page_end = number_bits;
        if (page_start >= page_end) [[unlikely]] return 0;

        const size_t num_bits = page_end - page_start;

        if (num_bits == number_bits) [[unlikely]] return word;
    ...
    void IterateWords(size_t offset, size_t size, Func&& func) const {
        ...
        // First word (partial)
        {
            ...
            if constexpr (BOOL_BREAK) {
                if (func(current_word, first_mask)) [[unlikely]] return;
            } else {
        ...
        for (; current_word + 3 < last_full_word; current_word += 4) {
            if constexpr (BOOL_BREAK) {
                if (func(current_word, full_mask)) [[unlikely]] return;
                if (func(current_word + 1, full_mask)) [[unlikely]] return;
                if (func(current_word + 2, full_mask)) [[unlikely]] return;
                if (func(current_word + 3, full_mask)) [[unlikely]] return;
            } else {
