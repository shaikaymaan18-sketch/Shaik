...
void TextureCacheRuntime::ClearImage(Image& image, u32 clear_value) {
    scheduler.RequestOutsideRenderPassOperationContext();
    
    VkClearColorValue clear_color{};
    clear_color.uint32[0] = clear_value;
    clear_color.uint32[1] = clear_value;
    clear_color.uint32[2] = clear_value;
    clear_color.uint32[3] = clear_value;
    
    VkImageSubresourceRange range{
        .aspectMask = VK_IMAGE_ASPECT_COLOR_BIT,
        .baseMipLevel = 0,
        .levelCount = static_cast<uint32_t>(image.info.resources.levels),
        .baseArrayLayer = 0,
        .layerCount = static_cast<uint32_t>(image.info.resources.layers)
    };
    
    scheduler.Record([image_handle = image.Handle(), clear_color, range](vk::CommandBuffer cmdbuf) {
        cmdbuf.ClearColorImage(image_handle, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 
                               clear_color, range);
    });
}
