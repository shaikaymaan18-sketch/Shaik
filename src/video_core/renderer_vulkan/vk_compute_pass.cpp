...
void RecordUnswizzleImageEntry(vk::CommandBuffer cmdbuf, VkPipeline pipeline, VkImage image,
...
void RecordUnswizzleImageExit(vk::CommandBuffer cmdbuf, VkImage image,
...
void BlockLinearUnswizzleImage2DPass::Unswizzle(
    ...
    scheduler.Record([vk_pipeline, vk_image, aspect_mask, is_initialized](vk::CommandBuffer cmdbuf) {
        RecordUnswizzleImageEntry(cmdbuf, vk_pipeline, vk_image, aspect_mask, is_initialized);
    });
    for (const VideoCommon::SwizzleParameters& swizzle : swizzles) {
        const size_t input_offset = swizzle.buffer_offset + map.offset;
    ...
    scheduler.Record([vk_image, aspect_mask](vk::CommandBuffer cmdbuf) {
        RecordUnswizzleImageExit(cmdbuf, vk_image, aspect_mask);
    });
    scheduler.Finish();
}
